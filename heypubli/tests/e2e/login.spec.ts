...
test.describe("login: password mode (/login?mode=password)", () => {
  const email = "e2e-login-pw@heypubli-e2e.test";
...
test.describe("login: 8-digit code", () => {
  const email = "e2e-login-code@heypubli-e2e.test";
  let userId: string;
...
test.describe("login: magic-link confirm page (/auth/confirm)", () => {
  const email = "e2e-login-link@heypubli-e2e.test";
  let userId: string;
  ...
  test("GET shows the button without consuming the token; clicking logs in", async ({
    ...
    const button = page.getByRole("button", { name: /sign in to heypubli/i });
  ...
  test("a stale token shows the error on the login page", async ({ page }) => {
    ...
    await page.getByRole("button", { name: /sign in to heypubli/i }).click();
