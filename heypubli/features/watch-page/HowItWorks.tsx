...
export function HowItWorks() {
  return (
    <div data-testid="watch-how" className="space-y-3">
      {t.how.steps.map((step, i) => (
        <div
          key={step.title}
          data-testid={`how-step-${i + 1}`}
          className="flex items-center gap-4 rounded-3xl bg-white p-5"
        >
          <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#1A1A1A] text-[15px] font-black text-white">
            {i + 1}
          </span>
          <div className="min-w-0 flex-1">
            <p className="text-[17px] font-black leading-snug text-[#1A1A1A]">
              {step.title}
            </p>
            <p className="mt-0.5 text-[14px] leading-snug text-[#6B7280]">{step.sub}</p>
          </div>
