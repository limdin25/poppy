// Draft the offer email for ONE house, from what we actually know about it.
//
...
export default async function handler(req: Request): Promise<Response> {
  ...
  if (body.callId) {
    try {
      ...
      // No transcript is a normal case, not an error. The email still writes.
    }
  }
  ...
  const out = await callLLM(MODEL, SYSTEM, [{ role: 'user', content: user }], 1200);
  if (!out) {
  ...
  const subject = m ? m[1].trim() : `Offer for ${h.address ?? 'the property'}`;
