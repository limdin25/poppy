...
async function handleWeb(req: Request): Promise<Response> {
  ...
  if (req.method === 'GET') {
    ...
    let q = sb
      ...
      .select('id, property_id, builder_id, contact_id, status, blocked_reason, body, sent_at, replied_at, confirmed_at, error, created_at, brrr_builders(name, phone), brrr_properties(address, wk_contact_id, viewing_at)')
