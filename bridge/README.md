...
| Ambient floor / comfort noise: the line never goes digitally dead, speaking OR listening | `telnyx.py` `RoomTone` + `mix_room_tone`: one stateful generator per call feeds both the between-turns comfort frames and the tone mixed UNDER every voice frame (Telnyx plays frames sequentially, so the floor can only survive her talking by being part of the voice audio); 50ms resume poll kills the end-of-turn gap | "the room tone rides UNDER the voice", "speak sends the audio chunked, level intact" |
...
| Giving up a stage never talks over her own question | `_giveup_line`: when the try that burned the stage was a question, the prospect gets one full listen before the give-up line, and a real answer is what gets replied to | "giving up a stage does not talk over a question she just asked" |
| No jumping the gun: the stability window reads the evidence | `settle_plan` in `agent.py`: a held contour (mid-thought) stretches the settled-partial wait to AssemblyAI's own end-of-turn patience; a landed one keeps the fast path with a higher word floor | "a held contour waits out the pause" |
| No input ignored: a grown sentence survives formatting | `_norm` + word-wise prefix in `assembly_stt._only_the_new_part`: the final's commas and capitals are not new speech, a real continuation goes through whole | "the answered-prefix test survives the final's punctuation" |
...
| Emotion as a spectrum, grounded, never manic | The cue system: intensity modifiers are first-class, `[emphasis]` mid-line, `[break]`/`[long-break]` for pauses; the ladder is capped ([extremely X] collapses to [very X]), two emotion cues per reply (`CUE_BUDGET`, mechanics exempt), and laughter written as words ("Haha") is swapped out in copy_guard | "the intensity ladder is capped", "two emotion cues a reply is the budget", "laughter written as words never reaches the voice" |
| Emotional inertia: a mood shift settles in, never snaps | the mood dict through `clean_cues`: a sharp valence flip renders the first cue of the new reply at base strength, one beat of the old energy ghosting | "a sharp mood flip loses its intensifier for one beat" |
| Gentle mirroring, never parody | `Prosody.energy()` reads the prospect's pitch spread; a flat quiet prospect pulls every intensifier off this reply; high energy changes NOTHING (softening is the only action) | "a flat quiet prospect pulls her intensity down", "the energy reader calls a flat line flat" |
| Fillers shaped by state, breath at the pivot | a slow-brain trip arrives behind a real `[break]` (struggle sounds different from flow); a `[break]` lands before the closing question of a multi-sentence reply, coin-gated, rendered by Fish as a natural intake, NEVER a synthesised breath noise | "a struggle trip carries a thinking pause", "a breath lands before the closing question" |
| No actions off silence: a booking needs a person who spoke | `booking_allowed` in `agent.py`: a `[BOOK:]` written in reply to "(they said nothing)" is refused and logged, never recorded | "a booking hallucinated against silence is never recorded" |
