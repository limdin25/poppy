-- Order the branch listings by the offer the engine actually emits.
--
-- valuation.py NESTS the band (deal.offer.max); the retired browser Comps page
-- flattened it (deal.offer_max). The RPC ordered on the flat key only, so for
-- every engine-valued row the sort key was NULL and "best deal first" silently
-- became "newest first". Same fault, same day, as headlineProperty() in
-- scripts/lib/property-branches.mjs, fixed together. Everything else in the
-- function is unchanged from 20260809000002.
--
-- Re-run safe.
...
language sql stable security definer set search_path = public as $$
  ...
  -- Best deal first, whichever shape the deal was stored in.
  order by coalesce(
...
comment on function public.wk_property_agent_listings(text) is
  'Staff-gated: every property this estate agency branch has listed, best offer first (nested deal.offer.max or legacy flat offer_max), with the deal jsonb and the live offer percentages. Read by usePropertyListings() for the dialer Houses tab. Matched on the last 9 digits of the phone so formatted and E.164 numbers agree.';
