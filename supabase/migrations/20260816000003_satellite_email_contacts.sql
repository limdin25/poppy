-- A rejection sat one contact over, and the deal said "no reply".
--
-- Orion Way, found by Hugo on 2026-08-16: "I think we got a reply from a
-- different email saying they don't accept, check." He was right. Lexi
-- Collins (lexi@ddmresidential.co.uk) wrote on the 14th at 08:38 that the
-- vendors of 39 Orion Way had REJECTED our offer, wanted closer to 110,000,
-- and invited an increase. Inbound email routing files a sender it does not
-- know onto an auto-created twin contact named after the address, the DDM
-- branch card had no email at all, and so the deal's state read "no reply
-- since the offer went out" for two days while the answer sat on the twin.
--
-- THE FIX: the messages a deal sees now include its SATELLITES, contacts
-- whose email shares the branch's company domain and who hold no properties
-- of their own (holding properties makes a same-domain contact a sibling
-- BRANCH of the same chain, with its own card, never merged). Freemail
-- domains are excluded outright, or a one-van agency on gmail.com would glue
-- every gmail sender in the CRM onto its card.
--
-- The freemail list must match FREEMAIL_DOMAINS in
-- api/lib/satellite-contacts.ts (the timeline's twin of this rule); a test
-- pins the two to each other.

create index if not exists wk_contacts_email_domain_idx
  on wk_contacts (split_part(lower(email), '@', 2))
