-- The confirmed builder rides on the card (Hugo, 2026-08-20: "we have to have
-- a way to merge the builder into the leads, so we know they are together...
-- they stick together and then they are synced").
--
-- The DATA merge already happens at the confirm press (assigned_builder_id on
-- the property, the outreach row, the stage move). What was missing is the
-- eye: wk_property_links gains ONE jsonb column, `builder`, so every surface
-- that draws a deal card (board, inbox header, call history) can glue the
-- builder to it with no extra round trip. Includes the builder's CRM contact
-- id so the chip can click through to their WhatsApp thread: the two threads
-- point at each other, which is what "synced" looks like in practice.
--
-- Same rules as 20260819000001: DROP first (the row type changes), and the
-- revoke at the bottom is load-bearing on a SECURITY DEFINER function.
...
language sql stable security definer set search_path = public as $$
  ...
  select
    ...
    -- The builder glued to the deal: only once one is CONFIRMED onto the
    -- viewing (assigned_builder_id is written by the confirm press alone).
    -- contact_id is the builder's own CRM thread, for the click-through.
    case when p.assigned_builder_id is not null then
      jsonb_strip_nulls(jsonb_build_object(
        'name',       bb.name,
        'phone',      bb.phone,
        'viewing_at', p.viewing_at,
        'contact_id', (
          select o.contact_id from brrr_builder_outreach o
          where o.property_id = p.id and o.builder_id = p.assigned_builder_id
          limit 1
        )
      ))
    end
  from brrr_properties p
  left join brrr_builders bb on bb.id = p.assigned_builder_id
...
comment on function public.wk_property_links(text[]) is
  'Staff-gated. Given many branch phone numbers, returns the LIVE houses each one has listed with a clickable Rightmove URL, the next-step brief, Hugo''s pinned note, the person we spoke to, `facts` (worth, band, comps tier, confidence, refurb, why), and `builder` (the CONFIRMED builder glued to the deal: name, phone, viewing_at, their CRM contact_id). Batched on purpose: the CRM board and Call history render a hundred rows at a time and must not make a hundred round trips. Matched on the last 9 digits so "0121 387 6499" and "+441213876499" agree.';
