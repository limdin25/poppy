-- The brain gets ears: the cockpit read now carries what was actually SAID.
--
-- Hugo, 16 Aug, looking at Paterson Road: "that one was wrong. The call is
-- there. The call is being recorded. Pedro asked as well a few questions."
--
-- He was right. Pedro had a 12 minute discovery call with Reeds Rains on the
-- 14th (117 transcript lines, outcome pressed, his own note saying "call back
-- monday"), and the brain ordered him to ring back and re-ask the discovery
-- questions, on a Sunday, before the agreed Monday. The grounds it used were
-- "0 of 12 checklist facts on file", which was true of the FILE and false of
-- the WORLD: the answers live in the transcript, the checklist only fills in
-- when the Houses-tab outcome form is used, and Pedro dispositions in the
-- dialer instead. The brain trusted the file because the file was all it
-- could see.
--
-- So wk_deal_cockpit_rows now carries two more things:
--   1. each call's agent_note ("call back monday" is an appointment, not
--      decoration), and
--   2. the FULL TRANSCRIPT of the newest call that has one, speakers named
--      for the reader (agent = Pedro, caller = the branch).
--
-- buildDealState() caps the transcript and the brain is told it is the ground
-- truth of the conversation. The prompt rules that use this live in
-- api/lib/deal-brain.ts (PROMPT_VERSION 3).
...
comment on function public.wk_deal_cockpit_rows(int) is
  'Staff-gated. Every live property with its contact, board column, last 20 calls (with agent notes), last 30 messages, live follow-ups, and the transcript of the newest recorded call, in one round trip. Feeds buildDealState() for the Deal Cockpit.';
