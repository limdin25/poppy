...
- **The call, the email and the after-call brief (2026-08-14)** — full write-up in [docs/BRRR_STRATEGY.md](docs/BRRR_STRATEGY.md) sections 8b to 8e, engineering record in [docs/DECISIONS_LOG.md](docs/DECISIONS_LOG.md) rows 25 to 37:
  - **The offer strip FOLDS and starts folded** in the dialer so the script owns the screen. Call history leaves it open.
  - **Condition is four questions, not one** ("it needs a bit of work" is refused), and **water is its own question on every house** (leaks, staining, roof), with the rule that a leak is the reason the price comes down, never a reason to walk.
  - **The email is asked for separately and SENT while the branch is on the phone.** Its own tab, property calls only. The address types itself from what the coach hears (`api/lib/spoken-email.ts`, verbatim twin inside `wk-voice-transcription`, drift-pinned by a test). Two call-one emails: the video request, and "No video, just my address" for a branch that refused. **Call one's email can never carry a figure** and there are three fences behind that.
  - **Every outcome writes a next-step brief** onto `brrr_properties.brief` (`api/lib/next-step-brief.ts`, deterministic, figures read never derived) and that text is the notification. `pinned_note` is Hugo's own instruction and is never machine-written.
  - **Two silent bugs fixed:** `callLLM` read `content[0].text`, so `claude-sonnet-5`'s thinking block made every AI email draft fail from the day it shipped (`api/lib/anthropic-content.ts` now owns the read); and `draft-offer-email` queried columns `wk_live_transcripts` does not have, so offer emails never saw the call.
  - **EPC, measured 2026-08-14:** the free register matched 394 of 4,160 lookups (9.5%) and returns the floor area only, so asking the branch is the fallback for the other 90%.
