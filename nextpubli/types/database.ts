...
/** How far someone got through signup. Forward-only, see lib/data/signup-leads.ts. */
export type SignupLeadStage = "started" | "sent_to_instagram" | "connected";

/**
 * Someone who finished the three signup questions, whether or not Instagram followed.
 * `whatsapp` keeps the schema-wide column name; every label a person reads says Mobile.
 */
...
export interface Database {
  public: {
    Tables: {
      ...
      signup_leads: {
        Row: SignupLead;
        // email_normalized is generated by Postgres, so it is never written.
        Insert: Omit<
          SignupLead,
          "id" | "email_normalized" | "first_seen_at" | "created_at"
        >;
        Update: Partial<
